...
search_try(int *hist, pte_t *trial_in, float *score_best, pte_t *ptbest, int depth, int around) {
   ...
   // Copy the current trial partition table to local trial variable
   pt_copy (trial, trial_in);

   // Iterate through each partition, except the first and last
   for (c = 1; c < PROBS; c++) {
      // Skip if not the partition around the current index (if specified)
      if ((around >= 0) && (abs(c - around) != 1)) continue;

      // Try decreasing the minimum value of the current partition
      while (trial[c].vmin > trial[c-1].vmin) {
          trial[c].vmin -= 1;

          // If depth is less than the maximum allowed depth, recurse with increased depth
          if (depth < DEPTH_MAX) {
              search_try(hist, trial, score_best, ptbest, depth+1, c);
          } else {
              // Otherwise, evaluate the new configuration
              float score_new = pt_encoded_size(hist, trial);
              
              // If the new configuration has a better score, update the best score and partition table
              if (score_new < *score_best) {
                  pt_copy(ptbest, trial);
                  *score_best = score_new;
                  
                  // Print the new best configuration if verbose mode is enabled
                  if (verbose == 1) {
                      fprintf(stderr, "PTBEST: ");
                      pt_print(ptbest);
                  }
              }
          }
      ...
      // Try increasing the minimum value of the current partition
      while (trial[c].vmin < trial[c+1].vmin) {
          trial[c].vmin += 1;
